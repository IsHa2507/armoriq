...
import { rulesAPI, approvalsAPI, type BackendRule, type BackendApproval } from "@/services/api";
import type { Rule, Approval } from "@/types/rule";

// ── shape adapters ────────────────────────────────────────────────────────────

function toFrontendRule(r: BackendRule): Rule {
  return {
    id: r.id,
    name: r.name,
...
function toFrontendApproval(a: BackendApproval): Approval {
  return {
    id: a.approval_id,
    ruleId: "",
...
export async function createRule(
  ...
  const backendPayload: Omit<BackendRule, "id"> = {
    name: payload.name,
    type: "BLOCK_TOOL",
...
export async function updateRule(
  ...
  const updates: Partial<BackendRule> = {};
  if (payload.name !== undefined) updates.name = payload.name;
  if (payload.status !== undefined) updates.enabled = payload.status === "active";
